...
from providers.registry import REGISTRY_VERSION, build_providers
...
async def version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_message:
        count = len(build_providers())
        await update.effective_message.reply_text(
            f"Версия реестра: {REGISTRY_VERSION}\nПоставщиков в коде: {count}"
        )
...
def run() -> None:
    ...
    LOGGER.info("Registry version=%s providers=%s", REGISTRY_VERSION, len(build_providers()))
